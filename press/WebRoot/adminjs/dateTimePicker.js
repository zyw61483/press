...
strFrame+='<span id=tmpSelectYearLayer  style="z-index: 9999;position: absolute;top: 3; left: 18;display: none"></span>';
strFrame+='<span id=tmpSelectMonthLayer  style="z-index: 9999;position: absolute;top: 3; left: 85;display: none"></span>';
strFrame+='<span id=tmpSelectHourLayer  style="z-index: 9999;position: absolute;top: 188; left: 3px;display: none"></span>';
strFrame+='<span id=tmpSelectMinuteLayer style="z-index:9999;position:absolute;top: 188; left: 56px;display: none"></span>';
strFrame+='<span id=tmpSelectSecondLayer style="z-index:9999;position:absolute;top: 188; left: 108px;display: none"></span>';
...
strFrame+='         onmouseover="style.color=\'#ff0000\'" onmouseout="style.color=\'#1478eb\'" title="当前日期时间">当前</span>&nbsp;<span style="cursor:hand; font-size:12px" id=evaAllOK onmouseover="style.color=\'#ff0000\'" onmouseout="style.color=\'#1478eb\'"  onclick="parent.closeLayer()" title="关闭日历">关闭</span>&nbsp;</td></tr>';
strFrame+='    </table></td></tr><tr ><td >';
strFrame+='        <table style="display:none" border=0 cellspacing=1 cellpadding=0 width=100%  bgcolor=#FFFFFF height=22 >';
strFrame+='          <tr bgcolor="#63A3E9">';
strFrame+='             <td style="font-size:12px;cursor:hand" onclick="parent.tmpSelectHourInnerHTML(this.innerText.length==3?this.innerText.substring(0,1):this.innerText.substring(0,2))"';
...
strFrame+='             <td style="font-size:12px;cursor:hand" onclick="parent.tmpSelectMinuteInnerHTML(this.innerText.length==3?this.innerText.substring(0,1):this.innerText.substring(0,2))"';
strFrame+=' onmouseover="style.backgroundColor=\'#aaccf3\'" onmouseout="style.backgroundColor=\'#63A3E9\'"';
strFrame+=' title="点击这里选择时间" align=center width=42>' ;
strFrame+='     <span id=meizzMinuteHead></span></td>';
strFrame+='             <td style="font-size:12px;cursor:hand" onclick="parent.tmpSelectSecondInnerHTML(this.innerText.length==3?this.innerText.substring(0,1):this.innerText.substring(0,2))"';
strFrame+=' onmouseover="style.backgroundColor=\'#aaccf3\'" onmouseout="style.backgroundColor=\'#63A3E9\'"';
strFrame+=' title="点击这里选择时间" align=center width=42>' ;
...
function getDateTime(tt)
{	
	meizzTheYear=new Date().getFullYear(); //定义年的变量的初始值
	meizzTheMonth=new Date().getMonth()+1; //定义月的变量的初始值
	meizzTheDate=new Date().getDate(); //定义日的变量的初始值 
	meizzTheHour=new Date().getHours(); //定义小时变量的初始值
	meizzTheMinute=new Date().getMinutes();//定义分钟变量的初始值
	meizzTheSecond=new Date().getSeconds();//定义秒变量的初始值
	setDateTime(tt);
}
...
function tmpSelectHourInnerHTML(strHour) //小时的下拉框
{
 ...
 for (var i = 0; i < 24; i++)
 {
  if (i == m) { selectInnerHTML += "<option value='"+i+"' selected>"+i+"时"+"</option>\r\n"; }
...

{
 ...
 for (var i = 0; i < 60; i++)
 {
  if (i == m) { selectInnerHTML += "<option value='"+i+"' selected>"+i+"秒"+"</option>\r\n"; }
